...
from puzzle_utils import batch_tch_divide_image, reconstruct_image_from_pieces
from dataset_builder import build_dataset, build_dataset_specific, build_dataset_pruned
...
def train(cfg):
    ...
    train_data = build_dataset_specific(cfg, cfg.digit)
    ...
    for epoch in range(1, cfg.epochs+1):
        ...
        for i, data in enumerate(train_loader):
            ...
            # Print the shape of the first sample (assuming all outputs are the same shape)
            # print(f"Shape of the first sample: {est_ordered_pieces[0].shape}")

            # If you want to print the shape of the entire list (it contains tensors)
            # print(f"Shape of the full output list (number of samples): {len(est_ordered_pieces)}")
    ...
    torch.save(model.state_dict(), os.path.join(cfg.out_dir, f"model_{cfg.tau}_{cfg.pieces}_{cfg.digit}.pth"))
...
def evaluation(cfg):
    ...
    model.load_state_dict(torch.load(os.path.join(cfg.out_dir, f"model_{cfg.tau}_{cfg.pieces}_{cfg.digit}.pth")))
    model = model.to(device)

    output_dir = f"output_images_{cfg.tau}_{cfg.pieces}_{cfg.digit}"  # You can specify any path where you want to save the images
    os.makedirs(output_dir, exist_ok=True)
    counter = 0
    eval_data = build_dataset_pruned(cfg)
    ...
    with torch.no_grad():
        ...
        for data in loader:
            ...
            for b in range(batch_size):
                ...
                original_reconstructed_image = reconstruct_image_from_pieces(pieces[b], perm_index, cfg.pieces, height, width)
                reconstructed_image = reconstruct_image_from_pieces(hard_sorted_pieces[b], est_perm_index, cfg.pieces, height, width)

                # Convert tensors to numpy arrays for saving
                original_reconstructed_image = original_reconstructed_image.cpu().numpy().transpose(1, 2, 0)
                reconstructed_image = reconstructed_image.cpu().numpy().transpose(1, 2, 0)

                # Save the original and reconstructed images
                original_image_filename = os.path.join(output_dir, f"original_image_{counter}.png")
                reconstructed_image_filename = os.path.join(output_dir, f"reconstructed_image_{counter}.png")
                counter += 1

                # Plotting the original and reconstructed images and saving them
                fig, ax = plt.subplots(1, 2, figsize=(10, 5))

                ax[0].imshow(original_reconstructed_image, cmap='gray')
...
if __name__ == "__main__":
    ...
    # Training option
    parser.add_argument("--digit", type=int, choices=range(10), default=0, help="Specify the digit (0–9) to filter from the MNIST training dataset")
