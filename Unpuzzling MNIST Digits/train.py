...
def train(cfg):
    ...
    torch.save(model.state_dict(), os.path.join(cfg.out_dir, f"model_{cfg.dataset}_{cfg.tau}_{cfg.pieces}.pth"))
...
def evaluation(cfg):
    ...
    model.load_state_dict(torch.load(os.path.join(cfg.out_dir, f"model_{cfg.dataset}_{cfg.tau}_{cfg.pieces}.pth")))
    model = model.to(device)

    output_dir = f"output_images_{cfg.dataset}_{cfg.tau}_{cfg.pieces}"  # You can specify any path where you want to save the images
    ...
    with torch.no_grad():
        ...
        for data in loader:
            ...
            for b in range(batch_size):
                ...
                original_reconstructed_image = reconstruct_image_from_pieces(pieces[b], perm_index, cfg.pieces, height, width, channels=3)
                reconstructed_image = reconstruct_image_from_pieces(hard_sorted_pieces[b], est_perm_index, cfg.pieces, height, width, channels=3)
                ...
                ax[0].imshow(original_reconstructed_image)
